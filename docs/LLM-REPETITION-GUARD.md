...
| 档 | 规则 | 抓什么 |
| --- | --- | --- |
| 主闸 | 真周期 k∈[6,64] 且含字母/CJK，连续 ≥6 次 | 长语/短语级循环（`研究研究到底`×6） |
...
- **pass-through 零扣留**：帧进帧出同 tick，不暂存不延迟，UI 打字机与流空闲看门狗无感；
  状态恒 ≤ 尾窗 384 字符（`MAX_UNIT × LONG_UNIT_MIN_REPEATS`——主闸最长游程必在窗内），
  每 delta 一次 O(窗长) 比对。
...
- k≥6 ×6：6 字以上单元原地连抄 6 次在专业正文构造不出合法用例（叠词封顶 ×2、引用样例
  通常 ×2~×6 且 k<6）；实测 13 例中 k≤5 占 13/13——主闸实际是面向未来长语循环的深闸。
...
`detect.test.ts`：两档触发全位（主闸下限位 k=6×6、保险丝交接带 Clean×20=span 100）、
