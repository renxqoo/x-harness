...
- **C1（中，已修）**：「根层∪本会话层」合并视图环检测落地（wouldCycleMerged——根段锚缺席名由会话层补名的跨层环注册期 throw；两条绕过路径封堵有专测）。附记：会话段锚名事后与本层新段名碰撞的形态**良定义**（分桶只认根层名，落尾稳定）——记为已知边缘不告警。
- **C2（低，已修）**：失败注册不留空层（sessionLayers.set 移至 registerIn 成功后）；层 Map 永不清理（无 sessionDisposed 接线）——零消费方下无可观察影响，挂账：首个消费者出现时接线。
