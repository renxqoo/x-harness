...
| 10 | @x-harness/tool-core + tool-read/tool-write/tool-bash/tool-grep：一命令一插件包（read/write/bash/grep + 后台任务登记簿） | docs/TOOLBOX.md | 已收口（grep rg 硬依赖 §5；run_in_background 登记簿 §4——task 动词归件14；一命令一包拆分形态见 §0） |
