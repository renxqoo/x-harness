...
```ts
// —— 上下文域 ——
transformMessages(ctx, fn: (entries: readonly InboxEntry[]) => InboxEntry[] | Promise<...>): Disposer;
  // F0① 语法糖：pre-step 改写（注入纠偏=往 entries 前插——inject 不单设，最少面）
transformAssistant(ctx, fn: (s: AssistantSettlement) => AssistantSettlement): Disposer;   // F0②
vetoStep(ctx, fn: (claim: PreStepClaim) => string | undefined): Disposer;                 // pre-step 否决（返回 reason=reject）
// —— 工具域 ——
vetoTools(ctx, fn: (req: ToolCallRequest) => Deny | undefined): Disposer;                  // tools/pre-execute
