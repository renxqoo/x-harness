...
- **/compact 语义**：统一走 compactionRunner（`@x-harness/compaction` 手动面）——结构化
  ...
  autocompact 分层防线（CP 后台账本维护 → L1 旧工具结果占位 → L2 账本落账——零 LLM
  前置层，主窗与 compactionKit 同一数字，CP 摘要面缺省回落 compactionRunner.summarizer；
  compaction 水位缺省 92% 强制压缩带，恒自主触发）。print 模式
