...
- **弱形态≠可用——workaround 的存在就是功能缺失的证据**：上游（pi-ai / adapter / 事件源）提供的数据，
  框架不得在中间层丢弃或折平。发现信息丢失（字段被合并/类型被擦除/数据被截断）时，它是 bug 不是
  「设计决策带绕行」。判据不是「缺它写不出来吗」（那问的是是否不可能），而是「数据是否完整正确地
  到达了需要它的地方」。典型犯错模式：
  - 「有间接指标够用」→ 实际是精确数据被折平（如 TokenUsage 丢 cacheRead/cacheWrite/cost）
  - 「宿主可以注入」→ 实际是运行时应该自己知道（如 contextWindow 不可查）
  - 「类型推导可得」→ 实际是上游有但没透传（如 totalTokens 被丢弃）
  - 「已知边界、文档言明」→ 实际是静默失败（如 softInject 拼错名无告警）
  - 「可以推导/绕行」→ 实际是类型级丢失（如 preStep messages 用 unknown[] 而非 SurfaceMessage）
