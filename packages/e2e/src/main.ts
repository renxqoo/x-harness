// e2e 入口：编排真实场景——插件动态注册/销毁（sqlite CRUD）+ agent 全链旅程 + 子代理旅程 + toolbox 四工具旅程（P12 进默认门）
// + CLI 宿主子进程旅程（docs/CLI.md §4 批E）。任一场景失败 → 非零退出码（check 脚本的 e2e 门）。
// 真凭证旅程见 `bun run e2e:real`（opt-in）。
...
try {
  ...
  await runCliJourney();
  console.log("\n场景通过：动态注册 / 使用 / 销毁无法使用 / 重装复活 / agent 全链 / 子代理旅程 / toolbox 四工具 / 跨进程双宿主 / worktree / 复活 / CLI 宿主子进程\n");
} catch (error) {
