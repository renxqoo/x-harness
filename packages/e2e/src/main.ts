...
try {
  ...
  let rows = read.all();
  must(
    rows.length === 2 && rows[0]?.name === "ada" && rows[1]?.name === "grace",
    `增+查生效（实际 ${JSON.stringify(rows)}）`,
  );
  ...
  rows = read.all(); // 未销毁的照常工作
  must(rows.length === 1, "销毁 crud-delete 后其余插件照常");

  // —— 销毁：其余三个也卸掉，全部无法使用，登记清空 ——
  ...
  // —— 复活：重新动态注册，立刻能用；sqlite 数据仍在（数据比插件活得久）——
  await install(world, "crud-create.ts");
  await install(world, "crud-read.ts");
  await install(world, "crud-delete.ts");
  const revivedRead = service<Read>(world, "crud-read");
  rows = revivedRead.all();
  must(
    rows.length === 1 && rows[0]?.name === "ada lovelace",
    `重装后数据仍在（实际 ${JSON.stringify(rows)}）`,
  );
  service<Create>(world, "crud-create").insert("linus");
  rows = revivedRead.all();
  must(
    rows.length === 2 && rows[1]?.name === "linus",
    `重装后写入立刻生效（实际 ${JSON.stringify(rows)}）`,
  );
  service<Delete>(world, "crud-delete").remove(rows[1]?.id ?? 0); // 复活的删同样立即可用
  rows = revivedRead.all();
  must(
    rows.length === 1 && rows[0]?.name === "ada lovelace",
    `复活的删立刻生效（实际 ${JSON.stringify(rows)}）`,
  );
