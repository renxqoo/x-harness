...
describe("放宽锚（防退回假阳性）", () => {
  it("算术展开 $((1+2))：非注入——auto 界内 ask（dynamic 词）、full allow（旧两档恒 ask）", () => {
    ...
    expect(adjudicateBash({ ...fenced, command: "echo $((1+2))", profile: FULL_PROFILE }).verdict).toBe("allow");
  });
  ...
  it("glob：未引用通配 auto ask（现行收紧面不放宽）、引号内通允许、full 放", () => {
    ...
    expect(adjudicateBash({ ...fenced, command: "cat *.log", profile: FULL_PROFILE }).verdict).toBe("allow");
  });
...
describe("reason 快照（§14.5-6——防实现期 reason 词漂移）", () => {
  it("全 reason 词表逐条钉死", () => {
    const PROFILE_OF: Record<"auto" | "full" | "plan", PermissionProfile> = { auto: AUTO_PROFILE, full: FULL_PROFILE, plan: PLAN_PROFILE };
