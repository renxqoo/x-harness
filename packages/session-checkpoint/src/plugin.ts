// 语义持久检查点（docs/SESSION-CHECKPOINT.md §1）：模型请求前与工具副作用前 flush 屏障，fail-closed。
// 挂点=agentRequest（P2/P3 裁决：此时 system/user 已落账，已领输入在日志）与 toolsExecute
// （tool/call 记录先于工具体持久——崩溃后可判别「已派发 outcome unknown」vs「未启动」，防双重副作用）。
// flush 失败一律 throw（不借「未调 next」违约文本当控制流）：请求侧逃逸 driver 收 error turn/end；
// 工具侧被 dispatch 管线收敛为携带 reason 的 isError outcome——两边界下游均零执行。
