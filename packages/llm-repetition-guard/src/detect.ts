...
//  - 主闸：单元真周期 k∈[6,64] 且含字母/CJK，连续 ≥6 次（跨度 ≥36）
...
// 状态恒 ≤ 尾窗 MAX_UNIT×6=384 字符（主闸最长单元 ×6 的完整游程必须落在窗内）——
...
/** 主闸：长单元（k≥6）触发次数——6 字单元原地连抄 6 次在专业正文构造不出合法用例 */
export const LONG_UNIT_MIN_REPEATS = 6;
/** 主闸单元长度下限（k<6 归保险丝档） */
export const LONG_UNIT_MIN = 6;
/** 保险丝：短单元（k∈[2,5]）触发跨度（×20~×50 次）——只止损 token 烧穿 */
export const SHORT_UNIT_SPAN = 100;
/** 尾窗长度：主闸最长单元（64）× 触发次数（6）——窗短于此则长单元游程被截不可判 */
