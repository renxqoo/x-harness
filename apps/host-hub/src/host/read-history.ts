...
export async function fenceSessionPath(sessionPath: string, sessionsRoot: string): Promise<FenceResult | { ok: false; reason: string }> {
  ...
  if (real !== undefined && !real.startsWith(`${root}/`)) {
    return { ok: false, reason: "session path outside sessions dir: symlink escape" };
  }
...
export function createDirectRead(deps: DirectReadDeps) {
  ...
  return {
    ...
    async readEntries(threadId: string, query: { since?: number; before?: number; limit?: number }) {
      ...
      return window.ok ? { entries: window.entries, leafSeq: window.leafSeq, hasMore: window.hasMore } : { error: window.reason };
    },
