...
| permission/set_mode/get_mode | 词表 `plan\|auto\|edit-confirm\|full\|sandboxed-auto`（源 plan\|default\|acceptEdits\|fullAuto）；get_mode 响应携 `modes` 词表（UI 选择器渲染源——单源）；即时切 = decide 面 + grants 授权面原子同步（源 controller 仅 decide 面——升级）；其余语义同构（即时/落盘/四态 source；controller.set 后置于 flush 成功） | 内核 ModeKnob + 服务面 |
